...
export default function SnippetsManagement() {
    ...
    const fetchSnippets = async () => {
        try {
            ...
            setSnippets(data);
        } catch (error) { toast.error('Failed to load snippets'); }
                        ...
                        snippets.map(s => (
                            <div key={s.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden group hover:shadow-md transition-all">
                                <div className="aspect-video bg-slate-50 flex items-center justify-center p-6 border-b border-slate-100 relative">
                                    ...
                                    <span className="absolute top-3 right-3 px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-white text-indigo-600 shadow-sm border border-indigo-100">{s.category}</span>
                                </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] w-full max-w-6xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col h-[90vh]">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex bg-slate-100 p-1 rounded-full">
                                    <button onClick={() => setViewMode('visual')} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${viewMode === 'visual' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}>Visual</button>
                                    <button onClick={() => setViewMode('code')} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${viewMode === 'code' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}>Code</button>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-hidden flex">
                            <div className="w-72 bg-slate-50 border-r border-slate-100 p-6 overflow-y-auto space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">General Details</label>
                                    <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Snippet Name" className="w-full px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm" />
                                    <select value={category} onChange={e => setCategory(e.target.value)} className="w-full px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm mt-3">
                                        {['Intro', 'Content', 'Features', 'Contact', 'Footer', 'Header', 'CTA'].map(c => <option key={c} value={c}>{c}</option>)}
                                    </select>
                                </div>
                                <div className="text-[10px] text-slate-400 p-4 border-2 border-dashed border-slate-200 rounded-2xl leading-relaxed italic">
                                    Quick Tip: Select any element in the preview to duplicate or delete it. Double-click text to edit directly.
                                </div>
                            </div>
                        ...
                        <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-white">
                            <button onClick={closeModal} className="px-6 py-2 text-slate-500 font-bold">Cancel</button>
                            <button onClick={handleSubmit} className="px-10 py-2.5 bg-indigo-600 text-white font-black rounded-full shadow-xl hover:bg-indigo-700 transition-all">Save Changes</button>
                        </div>
                    </div>
                </div>
            )}
