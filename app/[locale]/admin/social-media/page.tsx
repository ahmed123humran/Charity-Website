            ...
            <div className="flex flex-wrap justify-between items-center gap-4">
                ...
                {(userRole === 'ADMIN' || userRole === 'EDITOR') && (
                    <button
                        ...
                        className="flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-primary/90 transition-all shadow-sm cursor-pointer"
                    >
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <code className="text-xs bg-slate-100 px-2 py-1 rounded text-primary">{item.url}</code>
                                    </td>
                                    <td className="px-6 py-4 text-end">
                                        <div className="flex justify-end gap-2">
                                            {(userRole === 'ADMIN' || userRole === 'EDITOR') && (
                                                <button onClick={() => openEditModal(item)} className="p-2 text-slate-400 hover:text-primary transition-colors cursor-pointer">
                                                    <Edit2 className="w-4 h-4" />
                                                </button>
                                            )}
                                            {userRole === 'ADMIN' && (
                                                <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-red-500 transition-colors cursor-pointer">
                ...
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl border border-slate-100 overflow-hidden">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            ...
                            <button onClick={closeModal} className="text-slate-400 hover:text-slate-600 text-2xl leading-none cursor-pointer">&times;</button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            <div className="flex items-center gap-6">
                                <div className="relative group/avatar">
                                    <div className="w-20 h-20 rounded-2xl bg-slate-100 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all group-hover/avatar:border-primary">
                                        {image ? (
                            ...
                            <div>
                                <label className="flex items-center gap-1.5 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                                    {t('socialUrl')}
                                    <span title="يمكنك كتابة رابط الحساب أو رقم الواتساب مباشرة (مثال: 966500000000)" className="cursor-help text-primary">
                            ...
                            <div className="flex gap-3 pt-6">
                                <button type="button" onClick={closeModal} className="flex-1 px-4 py-3 text-slate-500 font-bold hover:bg-slate-50 rounded-2xl transition-all cursor-pointer">{commonT('cancel')}</button>
                                <button type="submit" className="flex-1 px-4 py-3 bg-primary text-white font-black rounded-2xl shadow-xl shadow-indigo-200 hover:bg-primary/90 transition-all cursor-pointer">
